...
class Analysis(threading.Thread):
    ...
    # in frames
    def pulse_detect(self, fname, mode):
    ...
    def analyze(self, fname):
        ...
        if settings.FREQUENCY_SPLIT:
            ...
            info = {"low": self.analyze_single(fname + '_low.wav'), 
                    "mid": self.analyze_single(fname + '_mid.wav'), 
                    "high": self.analyze_single(fname + '_high.wav')}
    ...
    def analyze_single(self, fname):
        ...
        pulse_loc = self.pulse_detect(fname, settings.PULSE_TYPE)
        prev_point = 0
        for i in range (0, len(pulse_loc)-1):
            ...
            classifier = self.sound_analyze(TEMP_NAME, settings.ANALYSIS_MODE)
    ...
    if not settings.FREQUENCY_SPLIT:
        ...
        for fname in os.listdir('../data/structural'):
            if fname.endswith('.wav'):
                Analysis(os.path.abspath('../data/structural/' + fname)).start()

        return master_combine()
...
def data_gen_single():
    ...
    return master_combine() # return the associated data in a markov object
